return {
    descriptions = {
        Joker = {
            j_aiz_chill_joker = {
                name = "Chill Joker",
                text = {
                    "{X:mult,C:white}X#1#{} Mult divided",
                    "by game speed",
                    "{C:inactive}(Currently {X:mult,C:white}X#2#{C:inactive} Mult)",
                },
            },
            j_aiz_loudspeaker = {
                name = "Loudspeaker",
                text = {
                    "Gives {C:chips}Chips{} based",
                    "on {C:attention}Audio volume{}",
                    "{C:inactive}(Currently {C:chips}+#1#{C:inactive} Chips)",
                },
            },
        },
    },
    misc = {
        dictionary = {
            k_aiz_advance = "Advance!",
            k_aiz_promoted = "Promoted!",
            k_aiz_trolled = "Trolled!",
            k_aiz_squared = "Squared!",
            k_aiz_cancelled = "Cancelled!",
            k_aiz_knowledge_gained = "Knowledge Gained!",
            k_aiz_dinner_postponed = "Dinner Postponed!",
            k_aiz_destroy = "Destroyed!",
            k_aiz_light = "Light",
            k_aiz_dark = "Dark",
            k_aiz_new_hand = "New Hand!",
            k_aiz_odds_increased = "Odds Increased!",
            k_aiz_odds_doubled = "Odds Doubled!",
            k_aiz_kiki = "Kiki!",
            k_aiz_bouba = "Bouba!",
        },
        challenge_names = {
            c_aiz_penny = "In For a Penny...",
        },
    },
}
