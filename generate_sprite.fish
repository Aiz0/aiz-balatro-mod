...
set texture_dir $script_dir/site/textures
# make 2x pngs from aseprite files
for file in $script_dir/assets/*.aseprite
    set png_file (basename $file .aseprite).png
    aseprite -b $file --scale 2 --save-as $texture_dir/$png_file
end
set textures $texture_dir/*.png
set jokers (string match -r -e 'j_\d\d?' $textures)
set jokers_soul (string match -r -e 'j_s_\d\d?' $textures)
# create the 2x atlases
montage $jokers -tile 6x3 -geometry 100%x100% -background none $atlas_dir/jokers.png
montage $jokers_soul -tile 6x3 -geometry 100%x100% -background none $atlas_dir/jokers_soul.png
